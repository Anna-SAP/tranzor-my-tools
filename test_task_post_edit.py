...
class FetchMrTwoPathTests(unittest.TestCase):
    """``_fetch_mr`` must check the cheap GitLab fix-commit signal first and
    fall through to dashboard cases only when needed. Either path → True."""
    ...
    def setUp(self):
        ...
        # Mock the cheap MR-commits fix-commit probe (default: no fix commit).
        self._fix_patch = mock.patch(
            "gitlab_client.mr_has_lead_fix_commit", return_value=False,
        )
        self._fix = self._fix_patch.start()
        self.addCleanup(self._fix_patch.stop)

        # Mock the shared GitLab client so has_token() is True without HTTP.
        self._client = mock.MagicMock()
        self._client.has_token.return_value = True
        self._sc_patch = mock.patch.object(
            tpe, "_shared_gitlab_client", return_value=self._client,
        )
        self._sc_patch.start()
        self.addCleanup(self._sc_patch.stop)

    def test_fix_commit_hit_short_circuits(self):
        """When the MR has a fix commit, we must not call dashboard cases at
        all (avoids the heavy 1-2MB response)."""
        self._fix.return_value = True
        self.assertTrue(tpe._fetch_mr(("proj", 1066)))
        self._cases.assert_not_called()

    def test_falls_through_to_dashboard_cases(self):
        """No fix commit → check fixed_by_lead via dashboard cases."""
        self._fix.return_value = False
    ...
    def test_legacy_bare_iid_skips_fix_commit_path(self):
        """A bare mr_iid (the PR #72 shape) has no project_id, so only the
        dashboard-cases path runs — preserves backward-compat."""
        self._cases.return_value = {
            "mrs": [{"cases": [
                {"fixed_by_lead": "alice@example.com"},
            ]}],
