...
| Before | After |
|---|---|
| Tick 8 rows in the HTML report → open Tranzor Platform → copy a `String Key` → paste into search → fix → repeat 7 more times | Tick rows → click `↗ Send to Tranzor` → the task page opens, the userscript **drives Tranzor's own search box** with each unique String Key in turn → the matching rows render across all languages → they're auto-highlighted AND their checkboxes are auto-ticked → click Tranzor's `Batch Retranslate` per key. Cycle through unique keys via the sidebar's `🎯 Filter Tranzor by next key` button. |
...
5. **The matching rows are already highlighted AND ticked in Tranzor's own list.** Every selected `String Key` gets a green left stripe + soft green background AND Tranzor's native row checkbox is auto-ticked — so the platform's own `Batch Retranslate` button (or any other batch action) is immediately usable. The sidebar (right edge) is the control surface:
   ...
   - **`🎯 Filter Tranzor by next key`**: this is the workhorse button. It fills Tranzor's own search box with the next unique String Key in your selection, **clicks Tranzor's "All Languages" view toggle** so every language for that key renders on one screen, then ticks only the language rows that are in your envelope (not, e.g., en-GB if you didn't select it). After ticking, click Tranzor's `Batch Retranslate`; then click this button again to advance. The status counter shows `(1/2)`, `(2/2)`, etc.
   - **`🔄 Re-scan`**: re-applies the current filter and re-scans the DOM. A DOM observer also auto-re-scans whenever Tranzor renders new rows (e.g. when you paginate).
...
| Sidebar shows `0/22 on page · 0 ticked` and a yellow warning despite being on the right task | Tranzor's task page is one big paginated list — selected rows are scattered across pages. Auto-filter probably didn't fire (the search box couldn't be found) | The sidebar shows your selection's unique String Keys as grouped cards. Click **🎯 Filter Tranzor by next key** (or any group's filter button) — the userscript will fill Tranzor's search box and the matching rows will tick themselves. If the auto-filter does nothing, Tranzor's search input may have moved — check `CONFIG.SELECTORS.searchInput` in the userscript. |
