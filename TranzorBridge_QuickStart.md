...
5. **The matching rows are already highlighted AND ticked in Tranzor's own list.** Every selected `String Key` gets a green left stripe + soft green background AND Tranzor's native row checkbox is auto-ticked — so the platform's own `Batch Retranslate` button (or any other batch action) is immediately usable. The sidebar (right edge) is the control surface:
   ...
   - **`🎯 Filter Tranzor by next key`**: this is the workhorse button. It fills Tranzor's own search box with the next unique String Key in your selection, then ticks the rows that appear (all languages for that key on one screen). After ticking, click Tranzor's `Batch Retranslate`; then click this button again to advance to the next key. The status counter shows `(1/2)`, `(2/2)`, etc.
