...
**① Presentation — `tkinter`.** `export_gui.py` (the PyInstaller entry point) owns the root window, the notebook, app-wide state, auth bootstrap, and the bridge lifecycle, and hosts the **File Translation** core tab directly; `gui_tabs.py` hosts the other two core tabs (MR Pipeline, Quality Overview). Each of the remaining **fourteen** tabs lives in its own `gui_tab_*.py`, constructed as `Tab(frame, app)` inside a `try/except` so a failure only logs `[X tab] init failed` and leaves the rest of the app intact — **3 core + 14 optional = 17**. Shared UI: `advanced_filter.py` (the reusable filter panel) and `date_picker.py` (a dark calendar). Stage counterparts of MR Pipeline and Full Translations reuse the production panel with `base_url` pointed at `tranzor-platform-stage`.
